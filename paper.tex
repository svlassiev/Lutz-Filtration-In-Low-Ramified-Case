...
\paragraph{}
В данной работе мы будем работать с формальной группой конечной высоты. Обозначим $F(X,Y)\in\OK[X,Y]$ --- формальный групповой закон для формальной группы конечной высоты $h$, заданный над кольцом целых многомерного локального поля $K$.\\ 
...
где $c_i(X)\in\OK[|X|]^*$, $c_0(X)\equiv1\mod X$, $\alpha_0:=e_0>\alpha_1>\alpha_2>\linebreak>\dots>\alpha_k>\alpha_{k+1}:=0$, $0=m_0<m_1<m_2<\dots<m_k<m_{k+1}:=h$.
