...
В этом параграфе мы покажем, как элементы модуля $F(\ML)$ представляются в виде суммы вида ${\sum\limits_{s,r}}_F[p]^r_F(\varepsilon_s(\theta_{s,r}))$. Для этого проведём рассуждения аналогичные, сделанным в работе \cite{book2} для случая одномерного поля.

Возьмём $[p]_F(X) \in pX+\OK[[X]]X^2$ --- эндоморфизм умножения на $p$ формальной группы $F$. Мы рассматриваем формальную группу высоты $h$, поэтому $[p]_F(X) \equiv c_hX^{p^h} \mod(\MK,X^{p^h+1})$. Это означает, что $[p]_F(X)$ можно записать в следующем виде:
$$[p]_F(X) = c_0t_1^{e_{K,1}} \ldots t_n^{e_{K,n}}X + c_1t_1^{\alpha^1_1} \ldots t_n^{\alpha^1_n}X^{p^{m_1}} + \ldots + c_kt_1^{\alpha^k_1} \ldots t_n^{\alpha^k_n}X^{p^{m_k}} + c_hX^{p^h}.$$
При этом $0 < \alpha^k_i < \alpha^{k-1}_i < \ldots < \alpha^1_i < e_{K,i}$ для всех $1 \Leq i \Leq n$.
Здесь $(t_1,\ldots,t_n)$ --- система локальных параметров поля $K$, а $c_i \in \OK[[X]]^*$ и $\overline{\val_K}(p) = \overline{e_K} = (e_{K,1},\ldots,e_{K,n})$.

Дополним введённые обозначения: $m_0 := 0$, $m_{k+1} := h$, $\alpha_i := (\alpha^i_1,\ldots,\alpha^i_n)$, $\alpha_{k+1} := (0,\ldots,0)$, $\alpha_0 := (e_{K,1},\ldots,e_{K,n})$. Тогда мы имеем $0=m_0<m_1<\ldots<m_k<m_{k+1}=h$ и $\alpha_{k+1} < \alpha_{k} < \ldots < \alpha_1 < \alpha_0$ в посимвольном смысле. Далее в этом параграфе операции и сравнения для векторов $\alpha_i$, $\overline{e}$, $\overline{e_K}$ и других производятся посимвольно.

В соответствии с работой \cite[Лемма 1]{book2} для каждого $1 \Leq i \Leq k+1$ рассмотрим вектор $e_*^{(i)} := {{\overline{e}} \over {\overline{e_K}}} {{\alpha_{i-1}-\alpha_i} \over {p^{m_i}-p^{m_{i-1}}}}$. Тогда, если для всех $i$ верно $e_{K,i} \Leq p$, то $e_*^{(1)} > e_*^{(2)} > \ldots > e_*^{(k+1)}$.

Учитывая вышеизложенное, можно записать следующие сравнения для действия $[p]_F(X)$ на элемент $a \in F(\ML)$:
\begin{enumerate}
	\item $0 < \overline{\val_L}(a) < e_*^{(k+1)}$, тогда $[p]_F(a) \equiv c_h a^{p^h} \mod deg(p^h\overline{\val_L}(a))$, 
	\item $e_*^{(i+1)} < \overline{\val_L}(a) < e_*^{(i)}$ для $1 \Leq i \Leq k$, тогда $[p]_F(a) \equiv \pi^{\alpha_i}c_i a^{p^{m_i}} \mod deg(\alpha_i{\overline{e} \over \overline{e_K}}+p^{m_i}\overline{\val_L}(a))$,
	\item $e_*^{(1)} < \overline{\val_L}(a)$, тогда $[p]_F(a) \equiv p c_0 a \mod deg(\overline{e}+\overline{\val_L}(a))$,
	\item $\overline{\val_L}(a) = e_*^{(i)}$ для $1 \Leq i \Leq k+1$, тогда\\ $[p]_F(a) \equiv t_1^{\alpha^{i-1}_1} \ldots t_n^{\alpha^{i-1}_n}c_{i-1}a^{p^{m_{i-1}}} + t_1^{\alpha^i_1} \ldots t_n^{\alpha^i_n} c_{i}a^{p^{m_{i}}} \mod deg(\alpha_{i}{\overline{e} \over \overline{e_K}}+e_*^{(i)}p^{m_{i}})$.	
\end{enumerate}

С помощью этих сравнений получим образующие для формального модуля $F(\ML)$ как $\Zp$-модуля. Пусть $\theta$ из $\RL$, тогда с помощью $\varepsilon_s(\theta)$ сопоставим $\theta$ некий элемент из $F(\ML)$ такой, что $\varepsilon_s(\theta) \equiv \theta T_1^{s_1} \ldots T_n^{s_n} \mod deg(s)$. 
Для начала индукцией проверим, что любой элемент $a$ из $F(\ML)$ можно представить в виде суммы следующего вида: $a={\sum\limits_{s=1}^\infty}_{(F)} \varepsilon_s(\theta_s)$, где $\theta_s \in \RL$, $\varepsilon_s(\theta_s) \in F(\ML)$ такие, что $\varepsilon_s(\theta_s) \equiv \theta T_1^{s_1} \ldots T_n^{s_n} \mod deg(s)$.\\
??? \cite{book7} ??? База индукции очевидна, так как любой элемент из $F(\ML)$ имеет вид $\alpha=\theta_1\Pi+\dots\ \Rightarrow \alpha\equiv\theta_1\Pi\mod\Pi^2$.\\
...
\begin{thebibliography}{3}   
    ...
    \bibitem{book7}И. Б. Жуков, А. И. Мадунц, <<Аддитивные и мультипликативные разложения в многомерных локальных полях>>, \begin{it}Вопросы теории представлений алгебр и групп. 7\end{it}, Зап. научн. сем. ПОМИ, \begin{bf}272\end{bf}, ПОМИ, СПб., 2000, 186–196.
